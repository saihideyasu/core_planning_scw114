...
private:
  ...
  void otherPoseCallback(const geometry_msgs::PoseStampedConstPtr &msg) const;
  ...
  geometry_msgs::PoseStamped other_pose_;
};
...
WaypointSaver::WaypointSaver() : private_nh_("~")
{
  ...
  other_pose_sub_ = nh_.subscribe("/predict_pose_for_ndt", 10, &WaypointSaver::otherPoseCallback, this);
  if (save_velocity_)
  {
...
void WaypointSaver::otherPoseCallback(const geometry_msgs::PoseStampedConstPtr &msg) const
{
  other_pose_ = *msg;
}
...
void WaypointSaver::outputProcessing(geometry_msgs::Pose current_pose, double velocity) const
{
  ...
  if (!receive_once)
  {
    ofs << "x,y,z,yaw,velocity,change_flag,other_pose" << std::endl;
    ...
    ofs << other_pose_.pose.position.x << "," << other_pose_.pose.position.y << "," << other_pose_.pose.position.z << std::endl;
    receive_once = true;
...
{
  ros::init(argc, argv, "waypoint_saver_scw");
  WaypointSaver ws;
  ros::spin();
  return 0;
}
