...
void callback_waypoint_param(const autoware_msgs::WaypointParam& msg)
{
  if(msg.lane_rule_deceleration >= 0) config_acceleration = msg.lane_rule_deceleration;
}
...
autoware_msgs::Lane apply_acceleration(const autoware_msgs::Lane& lane, double acceleration, size_t start_index,
                                       size_t fixed_cnt, double fixed_vel, bool reverse_flag)
{
  autoware_msgs::Lane l = lane;
...
autoware_msgs::Lane apply_crossroad_acceleration(const autoware_msgs::Lane& lane, double acceleration)
{
  ...
  for (const size_t i : end_indexes)
    l = apply_acceleration(l, acceleration, i, 1, l.waypoints[i].twist.twist.linear.x, false);
  ...
  for (const size_t i : reverse_start_indexes)
    l = apply_acceleration(l, acceleration, i, 1, l.waypoints[i].twist.twist.linear.x, true);
                                                ...
                                                size_t behind_cnt)
{
  ...
  for (const size_t i : indexes)
    l = apply_acceleration(l, acceleration, i, behind_cnt + 1, 0, false);
  ...
  for (const size_t i : reverse_indexes)
    l = apply_acceleration(l, acceleration, i, ahead_cnt + 1, 0, true);
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber waypoint_param_subb = n.subscribe("/waypoint_param", 10, callback_waypoint_param);
  ros::spin();
